...
export function CategoryDonut({ rows }: { rows: readonly CategoryRow[] }) {
  ...
  return (
    <div className="grid items-center gap-4 sm:h-[240px] sm:grid-cols-[minmax(0,1fr)_auto]">
      <div className="h-[200px] min-w-0 sm:h-full">
            ...
            <Tooltip
              content={({ active, payload }) => {
                const slice = payload?.[0]?.payload as
                  { name: string; amount: string } | undefined;
